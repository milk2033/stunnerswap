...
function App({
  ...
  const checkAllowance = async () => {
    const getAllowanceA = await usdc_contract.allowance(
      signer.address,
      amm.target
    );
    const getAllowanceB = await usdt_contract.allowance(
      signer.address,
      amm.target
    );
    await getAllowanceA.wait();
    await getAllowanceB.wait();
  ...
  const addLiquidity1 = async () => {
    ...
    if (reserve0 != 0) {
      ...
      const addLiquidity = await amm.addLiquidity(
        bigNumberA.toFixed(),
        bigNumberB.toFixed()
      );
    }
  ...
  const getAllowanceA = async () => {
    const getAllowanceA = await usdt_contract.allowance(
      signer.address,
      amm.target
    );
  ...
  const getAllowanceB = async () => {
    const getAllowanceB = await usdc_contract.allowance(
      signer.address,
      amm.target
    );
  ...
  const setUsdc = async (inputValueA) => {
    ...
    if (convert_reserve0 != 0) {
      ...
      const inputValueB = new BigNumber(
        Web3.utils.toWei(inputValueA * ratio, "ether")
      );
  ...
  const handleUsdtChange = async (event) => {
    console.log("usdt change hit");
    setInputA(event.target.value);
    console.log(inputA, "inputA");
    const inputValueA = new BigNumber(
      Web3.utils.toWei(event.target.value, "ether")
    );
  ...
  const approve_a = async () => {
    ...
    const approveA = await usdt_contract.approve(
      amm.target,
      a_converted.toFixed()
    );
  };
  const approve_b = async () => {
    ...
    const approveB = await usdc_contract.approve(
      amm.target,
      b_converted.toFixed()
    );
  };
    ...
    <div className="add-liquidity">
      ...
      <div className="selected-token-a">
        ...
        <div className="selected-token-a-input">
          <input
            onChange={(event) => handleUsdtChange(event)}
            placeholder="0"
          />
          <p style={{ cursor: "pointer" }} onClick={handleUsdtChange}>
      ...
      <div className="selected-token-b">
        ...
        <div className="selected-token-b-input">
          <input
            value={usdcValue}
            onChange={(event) => handleUsdcChange(event)}
            placeholder="0"
          />
          <p style={{ cursor: "pointer" }}>Balance: {usdc_balance}</p>
        </div>
      </div>
      <div>
      ...
      <div className="add-liquidity-approval-buttons">
        {isApprovedA ? null : <button onClick={approveA}>Approve USDT</button>}

        {isApprovedB ? null : <button onClick={approveB}>Approve USDC</button>}
