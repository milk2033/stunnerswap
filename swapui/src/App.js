...
function App() {
  ...
  const provider_test = new ethers.JsonRpcProvider(
  ...
  const usdcBalance = async () => {
    try {
      ...
      const getBalance =
        new BigNumber(await usdc_contract.balanceOf(signer.address)) / 10 ** 18;
  ...
  const usdtBalance = async () => {
    try {
      ...
      const getBalance =
        new BigNumber(await usdt_contract.balanceOf(signer.address)) / 10 ** 18;
