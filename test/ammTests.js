...
describe("AMM", function () {
    ...
    //     const usdc_allowance = await usdc.connect(address2).approve(amm.target, x.toString())
    //     const usdt_allowance = await usdt.connect(address2).approve(amm.target, x.toString())
    //     const mintUSDC = await usdc.mint(address2.address, x.toString())
    //     const mintUSDT = await usdt.mint(address2.address, x.toString())

    //     const startUSDCbalance = await usdc.balanceOf(address2.address)

    
    //     // const num0 = Web3.utils.toWei(x.toString(), 'ether')
    //     // const num1 = Web3.utils.toWei('1', 'wei')
    //     const converted_usdc = Web3.utils.toWei(1, 'ether')
    //     const converted_usdt = Web3.utils.toWei(1, 'ether')
    //     const BNusdc = new BigNumber(converted_usdc)
    //     const BNusdt = new BigNumber(converted_usdt)

    //     const addLiquidity = await amm.connect(address2).addLiquidity(BNusdc.toString(), BNusdt.toString())
        
    //     const finalUSDCbalance = await usdc.balanceOf(address2.address)
